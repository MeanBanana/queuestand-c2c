...
<body>
  ...
  <main>
    ...
    <?php if (empty($jobs)): ?>
      <p class="no-results">No open jobs at the moment. Check back soon!</p>
    <?php endif; ?>
  ...
  <script>
    ...
    function filterJobs() {
      ...
      cards.forEach(card => {
        const matches = card.dataset.title.includes(query) || card.dataset.location.includes(query);
      ...
      if (!noResultMsg) {
        ...
        jobsContainer.after(noResultMsg);
      }
      noResultMsg.style.display = (visible === 0 && query !== '') ? 'block' : 'none';
