...
if (!$job_id) { header('Location: dashboard.php'); exit; }

$stmt = $pdo->prepare("SELECT j.*, u.first_name, u.last_name, u.email FROM jobs j JOIN users u ON j.poster_id = u.user_id WHERE j.job_id = ? AND j.poster_id = ? AND j.status = 'assigned'");
...
<body>
    ...
    <div>
        ...
        <p>Amount: R<?= number_format($job['pay_amount'], 2) ?></p>
        <?php if ($alreadyPaid): ?>
            <p>✅ Payment complete. This job is now in progress.</p>
        <?php else: ?>
